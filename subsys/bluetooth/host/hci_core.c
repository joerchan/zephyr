...
static sys_slist_t adv_cb = SYS_SLIST_STATIC_INIT(&adv_cb);
static struct bt_adv adv_pool[CONFIG_BT_MAX_ADV];
#endif
...
static struct bt_adv *adv_new(void)
{
	struct bt_adv *adv = NULL;
	int i;

	for (i = 0; i < ARRAY_SIZE(adv_pool); i++) {
		if (!atomic_get(&adv_pool[i].ref)) {
			adv = &adv_pool[i];
			break;
		}
	}
...
struct bt_adv *bt_adv_ref(struct bt_adv *adv)
{
	...
	BT_DBG("handle %u ref %u", adv->handle, atomic_get(&adv->ref));
...
struct bt_adv *bt_adv_lookup_handle(u8_t handle)
{
	if (handle < ARRAY_SIZE(adv_pool) &&
	    atomic_get(&adv_pool[handle].ref)) {
		return bt_adv_ref(&adv_pool[handle]);
	}
...
struct bt_adv *adv_new_legacy(void)
{
#if defined(CONFIG_BT_ADV_EXT)
	if (bt_dev.adv) {
		return NULL;
...
struct bt_adv *bt_adv_lookup_legacy(void)
{
#if defined(CONFIG_BT_ADV_EXT)
	return bt_dev.adv;
#else
...
static int set_advertise_enable_ext(const struct bt_adv *adv, bool enable,
				    u16_t timeout, u8_t num_events)
{
	struct net_buf *buf;
	int err;
	...
	net_buf_add_le16(buf, sys_cpu_to_le16(timeout));
	net_buf_add_u8(buf, num_events);
...
static int set_adv_random_address(struct bt_adv *adv, const bt_addr_t *addr)
{
	struct net_buf *buf;
	struct bt_hci_cp_le_set_adv_set_random_addr *cp;

	if (!(IS_ENABLED(CONFIG_BT_ADV_EXT) &&
	      BT_FEAT_LE_ADV_EXT(bt_dev.le.features))) {
...
static int le_adv_set_private_addr(struct bt_adv *adv)
{
	bt_addr_t rpa;
	int err;
	...
	/* check if RPA is valid */
	if (atomic_test_bit(adv->flags, BT_DEV_RPA_VALID)) {
		return 0;
	}
...
static void le_update_private_addr(void)
{
	...
	if (atomic_test_bit(bt_dev.flags, BT_DEV_ADVERTISING) &&
		...
		struct bt_adv *adv = bt_adv_lookup_handle(0);

		set_advertise_enable_ext(adv, false, 0, 0);
		...
		// TODO: Privacy with duration and num_events.
		set_advertise_enable_ext(adv, true, 0, 0);
	}
#else /* defined(CONFIG_BT_ADV_EXT) */
	 ...
	 */
	err = le_set_private_addr(bt_dev.adv.id);
	if (err) {
		BT_WARN("Failed to update RPA address (%d)", err);
		return;
	}
...
#if defined(CONFIG_BT_ADV_EXT)
	atomic_clear_bit(adv_pool[0].flags, BT_ADV_RPA_VALID);
#endif
...
static int le_adv_set_private_addr(u8_t id)
{
	bt_addr_t nrpa;
	int err;
	...
	if (IS_ENABLED(CONFIG_BT_ADV_EXT) &&
	    BT_FEAT_LE_ADV_EXT(bt_dev.le.features)) {
		return le_set_private_addr(addr);
	}
...
bool bt_le_scan_random_addr_check(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_PRIVACY)) {
		...
		if ((atomic_test_bit(bt_dev.flags,
		     ...
		     bt_dev.id_addr[bt_dev.adv.id].type == BT_ADDR_LE_RANDOM) ||
		     bt_dev.adv.id != BT_ID_DEFAULT) {
			return false;
		}
	}
#endif /* !defined(CONFIG_BT_ADV_EXT) */
...
{
	if (IS_ENABLED(CONFIG_BT_ADV_EXT) &&
	    BT_FEAT_LE_SCAN_EXT(bt_dev.le.features)) {
...
int bt_le_create_conn_ext(const struct bt_conn *conn)
{
	...
	num_phys = (!(scan_param.options & BT_LE_CONN_OPT_NO_1M) ? 1 : 0) +
		   ((scan_param.options & BT_LE_CONN_OPT_2M) ? 1 : 0) +
		   ((scan_param.options & BT_LE_CONN_OPT_CODED) ? 1 : 0);
	...
	if (!(scan_param.options & BT_LE_CONN_OPT_NO_1M)) {
		cp->phys |= BT_HCI_LE_EXT_SCAN_PHY_1M;
		phy = net_buf_add(buf, sizeof(*phy));
		phy->scan_interval = sys_cpu_to_le16(scan_param.interval);
		phy->scan_window = sys_cpu_to_le16(scan_param.window);
		set_phy_conn_param(conn, phy);
	}

	if (scan_param.options & BT_LE_CONN_OPT_2M) {
		cp->phys |= BT_HCI_LE_EXT_SCAN_PHY_2M;
		phy = net_buf_add(buf, sizeof(*phy));
		// TODO: Should be ignored, but returned error
		phy->scan_interval = sys_cpu_to_le16(scan_param.interval);
		phy->scan_window = sys_cpu_to_le16(scan_param.window);
		set_phy_conn_param(conn, phy);
	}

	if (scan_param.options & BT_LE_CONN_OPT_CODED) {
		u16_t interval = scan_param.interval_coded ?
			scan_param.interval_coded : scan_param.interval;
		u16_t window = scan_param.window_coded ?
			scan_param.window_coded : scan_param.window;
...
static void enh_conn_complete(struct bt_hci_evt_le_enh_conn_complete *evt)
{
	...
	if (IS_ENABLED(CONFIG_BT_PERIPHERAL) &&
		...
		if (IS_ENABLED(CONFIG_BT_PRIVACY)) {
			// BUG: what if controller has local RPA == 0...0
			// TODO: check this with nRF Controller.
...
static void le_legacy_conn_complete(struct net_buf *buf)
{
	...
	// TODO: Assume that this is never called when AE is supported.
	struct bt_adv *adv = bt_adv_lookup_legacy();
	...
	if (IS_ENABLED(CONFIG_BT_PRIVACY)) {
		...
		// TODO: Does enh_conn_complete handle this in any way?
		bt_addr_copy(&enh.local_rpa, BT_ADDR_ANY);
	}

	if (evt->role == BT_HCI_ROLE_SLAVE) {
		id_addr = bt_lookup_id_addr(adv->id, &enh.peer_addr);
	} else {
			       ...
			       const bt_addr_le_t *addr, u8_t adv_props)
{
	...
	if (!(adv_props & BT_HCI_LE_EVT_TYPE_PROP_CONN)) {
		return;
	}
...
{
	switch (evt_type) {
		case BT_GAP_ADV_TYPE_ADV_IND: return 19; /* 0010011b */
		case BT_GAP_ADV_TYPE_ADV_DIRECT_IND: return 21; /* 0010101b */
		case BT_GAP_ADV_TYPE_ADV_SCAN_IND: return 18; /* 0010010b */
		case BT_GAP_ADV_TYPE_ADV_NONCONN_IND: return 16; /* 0010000b */
		 ...
		 */
		case BT_GAP_ADV_TYPE_SCAN_RSP: return 26; /* 0011010b */
		default : return 0;
	}
}

static void le_adv_recv(bt_addr_le_t *addr, struct bt_le_adv_info *info,
...
{
	switch (evt_type) {
		case 19: return BT_GAP_ADV_TYPE_ADV_IND;
		case 21: return BT_GAP_ADV_TYPE_ADV_DIRECT_IND;
		case 18: return BT_GAP_ADV_TYPE_ADV_SCAN_IND;
		case 16: return BT_GAP_ADV_TYPE_ADV_NONCONN_IND;
		case 26: /* Scan response from non-connectable advertiser */
		case 27: return BT_GAP_ADV_TYPE_SCAN_RSP;
		default : return 0;
	}
}
...
static void le_adv_ext_report(struct net_buf *buf)
{
	...
	while (num_reports--) {
		...
		adv_info.direct_addr = &evt->direct_addr;
		adv_info.primary_phy = evt->prim_phy;
		adv_info.secondary_phy = evt->sec_phy;
		adv_info.tx_power = evt->tx_power;
		adv_info.rssi = evt->rssi;
		adv_info.sid = evt->sid;

		adv_info.per_interval = evt->interval;

		adv_info.adv_type = get_adv_type(evt->evt_type);
		adv_info.adv_props = evt->evt_type;
...
static void le_adv_report(struct net_buf *buf)
{
	...
	while (num_reports--) {
		...
		adv_info.per_interval = 0;
		adv_info.direct_addr = NULL;
		...
		// TODO: handle ev_type incomplete or fragments
		le_adv_recv(&evt->addr, &adv_info, buf, evt->length);
...
static void le_adv_set_terminated(struct net_buf *buf)
{
	...
	struct bt_le_adv_cb *cb;
	struct bt_adv *adv;

	evt = (void *)buf->data;
	adv = bt_adv_lookup_handle(evt->adv_handle);

	BT_WARN("status 0x%02x adv_handle %u conn_handle 0x%02x num %u",
	...
	if (!evt->status) {
		...
		if (conn) {
			SYS_SLIST_FOR_EACH_CONTAINER(&adv_cb, cb, node) {
				cb->connected(adv, conn);
			}
	...
	// TODO: is num_sent set when connected?
	SYS_SLIST_FOR_EACH_CONTAINER(&adv_cb, cb, node) {
		cb->sent(adv, evt->num_completed_ext_adv_evts);
	}
...
#if 0 // CONFIG_BT_PER_ADV
	EVENT_HANDLER(BT_HCI_EVT_LE_PER_ADVERTISING_REPORT, le_adv_per_report,
		      sizeof(struct bt_hci_evt_le_per_advertising_report)),
	EVENT_HANDLER(BT_HCI_EVT_LE_PER_ADV_SYNC_ESTABLISHED,
	              le_adv_per_sync_established,
	               sizeof(struct bt_hci_evt_le_per_adv_sync_established)),
	EVENT_HANDLER(BT_HCI_EVT_LE_PER_ADV_SYNC_LOST, le_adv_per_sync_lost,
...
static int hci_set_ad_ext(struct bt_adv *adv, u16_t hci_op,
			  const struct bt_ad *ad, size_t ad_len)
{
	...
	// BUG: set_ad upstream always configures 32
	// TODO: sizeof(*set_param) is 255
	buf = bt_hci_cmd_create(hci_op, sizeof(*set_data));
	if (!buf) {
		return -ENOBUFS;
	}
...
static int set_ad(struct bt_adv *adv, const struct bt_ad *ad, size_t ad_len)
{
	if (IS_ENABLED(CONFIG_BT_ADV_EXT) &&
	    BT_FEAT_LE_ADV_EXT(bt_dev.le.features)) {
...
static int set_sd(struct bt_adv *adv, const struct bt_ad *sd, size_t sd_len)
{
	if (IS_ENABLED(CONFIG_BT_ADV_EXT) &&
	    BT_FEAT_LE_ADV_EXT(bt_dev.le.features)) {
...
int bt_set_name(const char *name)
{
	...
	if (adv && atomic_test_bit(bt_dev.flags, BT_DEV_ADVERTISING_NAME)) {
		...
		// BUG: App set scan data lost here.
		set_sd(adv, &sd, 1);
	}
...
#else
	// TODO check all advertising set
	struct bt_adv *adv = bt_adv_lookup_handle(0);
...
#else
	// TODO check all advertising sets
	struct bt_adv *adv = bt_adv_lookup_handle(0);
...
static int le_adv_update(struct bt_adv *adv,
...
int le_adv_set_random_addr(struct bt_adv *adv, u32_t options, bool dir_adv,
			   u8_t *own_addr_type)
{
	const bt_addr_le_t *id_addr;
	int err = 0;
	...
	if (options & BT_LE_ADV_OPT_CONNECTABLE) {
		...
		if (options & BT_LE_ADV_OPT_USE_IDENTITY) {
			...
			*own_addr_type = id_addr->type;
		} else if (!(IS_ENABLED(CONFIG_BT_ADV_EXT) &&
			    BT_FEAT_LE_ADV_EXT(bt_dev.le.features))) {
...
int bt_le_adv_start_ext(const struct bt_le_adv_param *param,
			const struct bt_data *ad, size_t ad_len,
			const struct bt_data *sd, size_t sd_len,
			const bt_addr_le_t *peer)
{
	...
	err = set_advertise_enable_ext(adv, true, 0, 0);
	if (err) {
		BT_ERR("Failed to start advertiser");
		if (IS_ENABLED(CONFIG_BT_PERIPHERAL) && conn) {
			bt_conn_set_state(conn, BT_CONN_DISCONNECTED);
	...
	// BUG: Advertising state might not be correct in connection complete.
	atomic_set_bit_to(bt_dev.flags, BT_DEV_KEEP_ADVERTISING,
			  !(param->options & BT_LE_ADV_OPT_ONE_TIME));
		     ...
		     const bt_addr_le_t *peer)
{
	if (IS_ENABLED(CONFIG_BT_ADV_EXT) &&
	    BT_FEAT_LE_ADV_EXT(bt_dev.le.features)) {
		return bt_le_adv_start_ext(param, NULL, 0, NULL, 0, peer);
	}

	return bt_le_adv_start_legacy(param, NULL, 0, NULL, 0, peer);
}
		    ...
		    const struct bt_data *sd, size_t sd_len)
{
	...
	if (IS_ENABLED(CONFIG_BT_ADV_EXT) &&
	    BT_FEAT_LE_ADV_EXT(bt_dev.le.features)) {
		return bt_le_adv_start_ext(param, ad, ad_len, sd, sd_len, NULL);
	}

	return bt_le_adv_start_legacy(param, ad, ad_len, sd, sd_len, NULL);
}
...
int bt_le_adv_stop(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_ADV_EXT) &&
		...
		err = set_advertise_enable_ext(adv, false, 0, 0);
		if (err) {
			return err;
		}
...
#else
	// TODO: Handle keep advertising feature for advertising sets.
#endif
}
...
{
	sys_slist_append(&adv_cb, &cb->node);
}
...
// void bt_adv_state_set(struct bt_adv *adv, new_state)
// {
// }
...
{
	struct bt_hci_cp_le_set_ext_adv_param *cp;
	// TODO: How to do directed advertising set?
	// Is advertising params in create_slave_le enough?
	bt_addr_le_t *peer = BT_ADDR_LE_ANY;
	bool dir_adv = false;//(peer != NULL);
	struct bt_adv *adv;
	struct net_buf *buf;
	int err;
	...
	if (!valid_adv_ext_param(param, false)) {
		BT_WARN("Invalid adv parameters");
		return NULL;
	}
	...
	adv->id = param->id;
	atomic_clear_bit(adv->flags, BT_DEV_RPA_VALID);
	...
	err = le_adv_set_random_addr(adv, param->options, dir_adv,
				     &cp->own_addr_type);
	if (err) {
		BT_WARN("Failed to set advertising random address");
		bt_adv_unref(adv);
		return NULL;
	}
	...
	if (param->options & BT_LE_ADV_OPT_PRIMARY_CODED) {
		cp->prim_adv_phy = BT_HCI_LE_PHY_CODED;
	} else {
		cp->prim_adv_phy = BT_HCI_LE_PHY_1M;
	}

	if (param->options & BT_LE_ADV_OPT_SECONDARY_CODED) {
		cp->sec_adv_phy = BT_HCI_LE_PHY_CODED;
	} else if (param->options & BT_LE_ADV_OPT_SECONDARY_CODED) {
		cp->sec_adv_phy = BT_HCI_LE_PHY_2M;
	} else {
		cp->sec_adv_phy = BT_HCI_LE_PHY_1M;
	}

	if (!((param->options & BT_LE_ADV_OPT_EXT_DATA) ||
	      (param->options & BT_LE_ADV_OPT_ANONYMOUS) ||
	      (param->options & BT_LE_ADV_OPT_USE_TX_POWER) ||
	      (param->options & BT_LE_ADV_OPT_PRIMARY_CODED))) {
		/* As long as we don't have extended length we use regular
		 * advertising.
		 */
		cp->props |= BT_HCI_LE_ADV_PROP_LEGACY;
	}
	...
	if (param->options & BT_LE_ADV_OPT_SCAN_REPORTS){
	...
	if (param->options & BT_LE_ADV_OPT_CONNECTABLE) {
		...
		if (dir_adv) {
			...
			// TODO: scannable should be set if legacy
		}
	}

	if (param->options & BT_LE_ADV_OPT_SCANNABLE) {
		// TODO: cannot be connectable if not legacy
		cp->props |= BT_HCI_LE_ADV_PROP_SCAN;
	}

	err = bt_hci_cmd_send_sync(BT_HCI_OP_LE_SET_EXT_ADV_PARAM, buf, NULL);
	if (err) {
		BT_WARN("Failed to set advertiser parameters");
		bt_adv_unref(adv);
		return NULL;
	}
	...
	// TODO handle name for advertising data
	// atomic_set_bit_to(bt_dev.flags, BT_DEV_ADVERTISING_NAME,
	// 		  param->options & BT_LE_ADV_OPT_USE_NAME);
			...
			u8_t num_events)
{
	...
	// TODO: Must fix if used for directed advertising.
	if (IS_ENABLED(CONFIG_BT_PERIPHERAL) &&
	...
	err = set_advertise_enable_ext(adv, true, timeout, num_events);
	if (err) {
		BT_ERR("Failed to start advertiser");
		if (IS_ENABLED(CONFIG_BT_PERIPHERAL) && conn) {
			bt_conn_set_state(conn, BT_CONN_DISCONNECTED);
...
int bt_le_adv_set_stop(struct bt_adv *adv)
{
	...
	return set_advertise_enable_ext(adv, false, 0, 0);
}
...
int bt_le_adv_set_delete(struct bt_adv *adv)
{
	...
	if (!BT_FEAT_LE_SCAN_EXT(bt_dev.le.features)) {
		return -ENOTSUP;
	}
...
#if defined(CONFIG_BT_OBSERVER)
static bool valid_le_scan_ext_param(const struct bt_le_scan_param *param)
{
	return true;
}
...
static bool valid_le_scan_param(const struct bt_le_scan_param *param)
{
	if (IS_ENABLED(CONFIG_BT_ADV_EXT) &&
            BT_FEAT_LE_SCAN_EXT(bt_dev.le.features)) {
		return valid_le_scan_ext_param(param);
	}
...
int bt_le_scan_start(const struct bt_le_scan_param *param, bt_le_scan_cb_t cb)
{
	...
	if (IS_ENABLED(CONFIG_BT_ADV_EXT) &&
		...
		if (param->options & BT_LE_SCAN_OPT_CODED) {
			...
			param_coded.interval = sys_cpu_to_le16(param->interval);
			param_coded.window = sys_cpu_to_le16(param->window);
